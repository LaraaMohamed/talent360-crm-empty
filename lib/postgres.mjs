 ...
 *
 * Comment-aware too, and not as a nicety: this codebase writes its longest
 * documentation directly inside `/* ... *\/` SQL comments (lib/calling.mjs's
 * `queue()` is one), in ordinary prose — contractions and all. A scanner
 * that only tracked `'`/`"`/`` ` `` quoting read a stray apostrophe in a
 * comment like "the call's own activity body" as the START of a string
 * literal, then kept scanning past the comment's real `*\/` looking for a
 * closing quote — swallowing everything up to the next unrelated `'` in the
 * query as "inside a string" and inverting every quote boundary after it,
 * which silently stopped rewriting `?` placeholders past that point. The
 * live symptom was `Error: syntax error at or near "AND"` from Postgres,
 * nowhere near any actual `AND` a person wrote. Comments have to be
 * recognized and skipped verbatim, the same as strings, before quote
 * tracking can be trusted at all.
 */
export function toPositional(sql) {
    ...
    for (let i = 0; i < sql.length;) {
        ...
        // `-- ...` to end of line: copied verbatim, never scanned for quotes
        // or `?` — a comment cannot contain a parameter marker.
        if (ch === '-' && next === '-') {
            flushWord();
        ...
        // `/* ... */`: same, up to (and including) the first `*/`. Not
        // nesting-aware — SQLite and this app's own comments never nest —
        // so an unterminated block comment is copied to the end of the
        // string rather than than risking a false close inside it.
        if (ch === '/' && next === '*') {
            flushWord();
