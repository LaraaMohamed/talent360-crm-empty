 ...
 *
 *   · No TURSO_URL — `node:sqlite`, one file on disk. Zero dependencies, and
 *     `crm.db` can be copied as a backup. This is how the CRM runs on a laptop
 *     and how `node test.mjs` runs.
 *
 ...
 *     the login with it.
 *
 * Both are SYNCHRONOUS, which is the whole point. See lib/turso.mjs for how the
 * remote one manages that and what it costs; the short version is that ~800
 * call sites and every `tx()` in this codebase keep meaning exactly what they
 * say, instead of being rewritten into promises that fail silently when one
 * `await` is missed.
 */
...
export function open() {
    ...
    backend = REMOTE ? openRemote() : openLocal();
    return backend;
}
...
export function migrate() {
    const conn = open();
    conn.exec(fs.readFileSync(path.join(ROOT, 'schema.sql'), 'utf8'));
