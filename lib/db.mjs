...
/**
 * True when this process is talking to ANY hosted database — Turso or
 * Postgres — rather than a local file. Several places in the app care about
 * this broader fact rather than which hosted backend specifically: the
 * first-run setup endpoint must fail closed without a token on any database
 * reachable from the open internet, and document storage must duplicate
 * uploaded bytes into the database itself on any host whose filesystem does
 * not survive a restart. `REMOTE` alone used to be all there was, and every
 * one of those call sites read it for exactly this meaning — this constant
 * keeps them all correct now that there are two hosted backends, not one.
 */
export const HOSTED = REMOTE || POSTGRES;
