...
function validateEnvironmentForDocType_(docType) {
  ...
  if (docType.requiresCommercialRegistration) {
    getSheet_(SHEETS.COMMERCIAL_REGISTRATION);
  }
}

/**
 * Confirms an Opportunity has the imported Commercial Registration data that
 * this document type depends on for its First Party (الطرف الأول) block.
 *
 * Registry-driven via `requiresCommercialRegistration`, so a future document
 * type opts in with one flag and gets this check for free.
 * @param {Object} docType One entry from DOCUMENT_TYPES.
 * @param {string} opportunityId
 * @return {Array<string>} Problems found; empty = valid.
 */
function validateCommercialRegistrationFor_(docType, opportunityId) {
  if (!docType.requiresCommercialRegistration) return [];

  if (!hasCommercialRegistration_(opportunityId)) {
    return ['A Commercial Registration certificate has not been imported for this Opportunity yet. ' +
      'Import it first so the First Party details (company name, registration number, representative, address) ' +
      'can be filled in automatically.'];
  }

  // The representative signs on the client's behalf ("ويمثلها قانوناً السيد /"),
  // and it is no longer a field on the generation form, so a blank one has to
  // be caught here rather than silently producing an agreement with a gap.
  var cr = getCommercialRegistration_(opportunityId);
  var rep = String(cr[COLUMNS.COMMERCIAL_REGISTRATION.REPRESENTATIVE_NAME] || '').trim();
  if (!rep) {
    return ['No representative name is recorded for this client. Open "Import Commercial Registration", ' +
      'pick or type the manager who signs on their behalf, and save.'];
  }
  return [];
}
