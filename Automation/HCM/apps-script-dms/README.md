...
5. **Generate Document** again on the same Opportunity, choose **HCM
   ...
   Article 1 is missing the Benefits sub-clause, with its numbering
   `1.`–`6.` running in document order with no gap either.

   (This step described the numerals as `١.`–`٦.` until 2026-08-07. They
   are Western digits: `numberFormat` returns the number and the template
   supplies the period. Two defects found at the same time — numbering by
   registry order rather than document order, and a doubled period — were
   fixed then; see the comments in `DocumentEngine.gs` and
