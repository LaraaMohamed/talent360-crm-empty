...
/**
 * Arabic (Levantine/Egyptian) month names, matching the style already used
 * in the Arabic agreement templates ("1 أبريل 2026م").
 */
var ARABIC_MONTHS = [
...
/**
 * Formats a Date the way the Arabic agreements write dates in prose, e.g.
 * "1 أبريل 2026م". Used for the contract start/end dates inside the Arabic
 * agreement body, where a bare dd/MM/yyyy would read wrong.
 * @param {Date} date
 * @return {string}
 */
function formatArabicDate_(date) {
  if (!(date instanceof Date)) return normalizeDateValue_(date);
  var tz = CONFIG.TIMEZONE;
  var day = Utilities.formatDate(date, tz, 'd');
  var monthIndex = parseInt(Utilities.formatDate(date, tz, 'M'), 10) - 1;
  var year = Utilities.formatDate(date, tz, 'yyyy');
  return day + ' ' + ARABIC_MONTHS[monthIndex] + ' ' + year + 'م';
}

/**
 * Returns today's date in the spreadsheet's configured timezone, with the
 * time component zeroed.
 *
 * Built from the timezone-formatted y/m/d rather than a raw `new Date()` so
 * a late-evening run near a UTC boundary can't stamp tomorrow's (or
 * yesterday's) date onto an agreement.
 * @return {Date}
 */
function todayInConfiguredTimezone_() {
  var now = new Date();
  var y = parseInt(Utilities.formatDate(now, CONFIG.TIMEZONE, 'yyyy'), 10);
  var m = parseInt(Utilities.formatDate(now, CONFIG.TIMEZONE, 'M'), 10);
  var d = parseInt(Utilities.formatDate(now, CONFIG.TIMEZONE, 'd'), 10);
  return new Date(y, m - 1, d);
}

/**
 * Computes a contract end date as "start + N years - 1 day", the standard
 * inclusive-term convention.
 *
 * Example: 04/05/2026 + 1 year - 1 day = 03/05/2027.
 *
 * Leap-year note: JS Date normalizes an invalid intermediate date, so a
 * 29 Feb start rolls to 1 Mar before the day is subtracted, giving
 * 28 Feb of the following year — the correct inclusive end date.
 * @param {Date} startDate
 * @param {number=} years Defaults to 1.
 * @return {Date}
 */
function computeContractEndDate_(startDate, years) {
  if (!(startDate instanceof Date)) return null;
  var n = years === undefined || years === null || isNaN(Number(years)) ? 1 : Number(years);
  var end = new Date(startDate.getFullYear() + n, startDate.getMonth(), startDate.getDate());
