 ...
 * Helpers.gs
 *
 * Generic, reusable utilities shared across the project: sheet access,
 * header-name-based column lookup, row <-> object conversion, and small UI
 * helpers. Nothing proposal-specific lives here.
 */
 ...
 * This is what lets every other file refer to columns by name instead of by
 * position, so reordering or inserting sheet columns never breaks the code.
 ...
 * completely empty; otherwise leaves existing headers untouched (adding
 * columns is a deliberate, user-driven action, not something we do silently
 * on every open).
 * @param {GoogleAppsScript.Spreadsheet.Sheet} sheet
 */
 ...
 * (i.e. keyed by the values in COLUMNS.*, not by internal short names).
 * @param {GoogleAppsScript.Spreadsheet.Sheet} sheet
 ...
 * client-supplied data (a company name, a fee string) must be doubled.
 */
...
/** Shows a friendly alert dialog. Falls back to a plain message if title is omitted. */
